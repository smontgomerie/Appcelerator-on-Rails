...
var Video = Model.extend({
    table_name: "videos",
    _fields: {id: Number, filename: String, title: String, created_at: Date, category: String, thumbnail: String},
    find: function(id) {
        var activity = new Video(this.db, this._find(id));
        return activity;
    },

    item_from: function(row) {
        var activity = new Video(this.db, this._item_from(row));
        return activity;
    }
});
...
var User = Model.extend({
    table_name: "users",
    _fields: {version: String},
    find: function(id) {
        var activity = new User(this.db, this._find(id));
        return activity;
    },

    item_from: function(row) {
        var activity = new User(this.db, this._item_from(row));
        return activity;
    }
});